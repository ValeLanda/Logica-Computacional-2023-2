...
--Contar operadores
--ExpB
countExpB :: ExpB -> Intmport Data.Char
...
eval (Sum x y) = x + y
eval (Mult x y) = x * Y
eval (IfThEl p x y ) = if evalB(b) then eval(x) else eval(y) 

--ExpB
evalB :: ExpB -> Bool
evalB V = True
evalB F = False
evalB (Not p) = not (evalB(x))
evalB (Or p q) = evalB(x) || evalB(y)
evalB (And p q) = evalB(x) && evalB(y)
evalB (Equal p q) = evalB(x) == evalB(y) 
